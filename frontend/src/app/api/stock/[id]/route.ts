...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase().from('StockItem').select('*').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const { data: existing } = await supabase().from('StockItem').select('id').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
  ...
  const { data } = await supabase().from('StockItem').update(update).eq('barbershopId', barbershopId).eq('id', id).select().single()
  return json(data)
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const barbershopId = user.barbershopId
  const { id } = await params
  await supabase().from('StockItem').delete().eq('barbershopId', barbershopId).eq('id', id)
