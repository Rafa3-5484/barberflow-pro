...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const { data: existing } = await supabase().from('Appointment').select('id').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
  ...
  const { data } = await supabase().from('Appointment').update(update).eq('barbershopId', barbershopId).eq('id', id)
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await supabase().from('Appointment').select('id').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
  if (!existing) return error('Agendamento não encontrado', 404)
  await supabase().from('Appointment').delete().eq('barbershopId', barbershopId).eq('id', id)
