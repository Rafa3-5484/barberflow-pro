...
export async function generateReport(startDate: Date, endDate: Date) {
  ...
  const servicosMaisVendidos = Object.entries(
    completed.reduce<Record<string, { name: string; count: number; total: number }>>((acc, a) => {
  ...
  ).map(([hora, total]) => ({ hora, total })).sort((a, b) => b.total - a.total).slice(0, 10)

  const comissaoPorProfissional = await Promise.all(
    [...new Set(completed.map(a => a.professionalId))].map(async (profId) => {
      const profAppointments = completed.filter(c => c.professionalId === profId)
      const revenue = profAppointments.reduce((s, c) => s + (c.service_price || 0), 0)
