...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: open } = await supabase().from('CashRegister').select('id').eq('barbershopId', barbershopId).eq('status', 'OPEN').maybeSingle()
    ...
    const { data, error: dbErr } = await supabase().from('CashRegister').insert({
      ...
      openedAt: new Date().toISOString(), status: 'OPEN', barbershopId,
    }).select().single()
