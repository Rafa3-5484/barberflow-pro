...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase().from('Professional').select('*, "Appointment"(*, "Client"(*), "Service"(*))').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const { data: existing } = await supabase().from('Professional').select('id').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
  ...
  const { data } = await supabase().from('Professional').update(update).eq('barbershopId', barbershopId).eq('id', id).select().single()
  return json(data)
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await supabase().from('Professional').select('id').eq('barbershopId', barbershopId).eq('id', id).maybeSingle()
  if (!existing) return error('Profissional não encontrado', 404)
  await supabase().from('Professional').delete().eq('barbershopId', barbershopId).eq('id', id)
